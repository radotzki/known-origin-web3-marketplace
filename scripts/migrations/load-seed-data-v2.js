      ...
      return getTransactionCount(_developerAccount)
        .then((accountNonce) => {
          ...
          return Promise.all(_.map(flatData, function ({costInWei, ipfsPath, edition}) {
            ...
            // Get the KO ABI
            const KnownOriginMarketPlace = web3.eth.contract(require('../abi/KnownOriginV1_ABI')).at(instance.address);

            // Generate the raw transaction
            const rawTransaction = KnownOriginMarketPlace.mint.request(tokenUri, edition, costInWei.toString(10), _openingTime, _artistAccount, {from: _developerAccount});

            // Send raw transaction, bumping the nonce for each once
            let response = sendTransaction({
              from: _developerAccount,
              to: instance.address,
              data: rawTransaction.params[0].data,
              gas: gas || 4075039,
